...
| CS-162 | **"Mark Simplify Bodies" — a marked subtree exports as ONE merged mesh (user-requested; Isaac Sim per-prim cost).** Two context-menu entries ("Mark Simplify Bodies" / "Clear Mark Simplify Bodies") on the component tree of **GENERATED models only** (assets + Static; the exact mirror of the Source-only `is_asset` gate). **As-built:** `NodeConfig.simplify_bodies` + `SceneConfig.simplify_ids()` (no `SCHEMA_VERSION` bump — `exclude_defaults=True` minimization makes a `bool=False` field free); new pure `src/model/simplify_marks.py` (`SimplifyBlocker`, `validate_marks`, `require_valid_marks`, `resolve_simplify_groups`, `swallowed_ids`, `marked_ancestor`); `usd_writer._author_mesh` split into `_baked_tris` + `_write_mesh_prim` + new `_author_merged_mesh`, with `simplify_cids` threaded through `_author_components` → both `write_subtree_usd` and `write_model_usd` (so subtree AND composed export are covered by ONE insertion point); `obj_writer` emits one `o` group per mark; `main_window._simplify_nodes` / `_set_simplify_nodes` / `_confirm_frame_breaks_simplify` (guards Create Joint + ReOrigin); `tree_panel` `SIMPLIFY_ROLE` = `UserRole+12`, cyan stacked-bars glyph. ⭐ **`src/export/frame_paths.py` MOVED to `src/model/live_frames.py`** (user-approved layer call) + gained `live_frames_and_joints()` / `joint_body_cids()`: the GUI must validate against the SAME live-frame set the exporters use, and `src/gui/` must not import `src/export/`. `usd_cli`/`obj_cli` now share that derivation. **Decisions (ADR-0010):** triangle concatenation not `BRepAlgoAPI_Fuse`; one mesh with per-triangle `uniform` displayColor (color-lossless — this writer has no UsdShade/GeomSubset to lose); USD + composed USD + OBJ, **not** STEP; triangle count unchanged (no weld, no decimate); a mark is refused when a **STRICT** descendant owns a live frame or is a joint body (the marked node's OWN frame is fine — marking a link that IS the joint's Body1 is the primary workflow); a stale mark is **fatal on every export path** (deliberate divergence from ADR-0007's tolerant stance — see the ADR). **Verified headless (48 checks, 3 probes):** `scratchpad/simplify_marks_probe.py` (synthetic — validation matrix, grouping, USD prim/triangle/color/joint/customData invariants, monochrome→`constant` collapse, empty-mark byte-neutrality, path-keyed config round-trip); `simplify_obj_gui_probe.py` (OBJ grouping + `v`/`f` bytes identical; offscreen `QWidget.grab()` PNG read back — glyph paints as three bars, image-diff confined to the marked row, other glyphs untouched); `simplify_real_probe.py` (**real testB2**: Static **1860 mesh prims → 1106**, 755 bodies folded, 3.78 M triangles and 924 k per-triangle colors identical; `asset:Butler` 7 joints + their Body0/Body1 targets unchanged with a link marked, and a mark containing a jointed link correctly fatal). ❓ **USER live-test pending:** the two menu entries' enable/disable on a mixed multi-selection, the two warning dialogs, and the actual Isaac Sim frame-rate win that motivated the feature | live-test-pending | **P1** | agent | ADR-0010; `Reference/export.md` § *Simplify Bodies*, `node-state-and-color.md`, `kinematic-joints.md` |
| CS-161 | **✅ ROOT-CAUSED 2026-08-13 — see the resolution at the end of this entry; the narrative below is how it looked BEFORE the cause was found, and its "not reproducible" and "root cause unknown" claims are now SUPERSEDED.** **(was ⛔ UNEXPLAINED) My own `Assert both … artifacts were produced` step printed both `ok` lines and then exited 1.** Log: `ok dist/CellSmith-v0.1.0-alpha.1-linux-x86_64.tar.gz (481M)` / `ok dist/cellsmith_0.1.0~alpha.1_amd64.deb (315M)` / `Error: Process completed with exit code 1`. Both branches took the success path, so `rc` was never set to 1 and no `::error::` was emitted — yet the step failed. **NOT REPRODUCIBLE LOCALLY:** the script extracted verbatim from the YAML and run as `bash -e -l script` in WSL, with files of the same names (including the `~` in the `.deb`), exits **0**. Ruled out by probe: CRLF (file and committed blob are LF-only, no `.gitattributes`), YAML mangling (extracted bytes match the file), a stray third loop iteration, and an `::error::` hidden in the annotation stream. **Fix = remove the two variables I could not inspect, and make the next occurrence decisive** rather than guess: the step now uses **`shell: bash`** (`bash --noprofile --norc -eo pipefail`) instead of `bash -el {0}` — it only globs and stats files, so loading the conda login profile was never justified — drops **`compgen -G`** for a plain glob loop, and **prints `verdict: rc=N` before exiting**. ⭐ If a future log shows `verdict: rc=0` and the step still fails, the cause is provably the shell wrapper and not the script; the first version could not distinguish those, which is what cost the round-trip. ⚠️ The glob loop uses `if [ -e "$f" ]; then …; fi`, **never** `[ -e "$f" ] && hit=$f` — under `-e` the no-match path would abort the script, the same trap as `CS-159`(2). **Verified:** both scripts extracted from the YAML and run under GitHub's exact `shell: bash` invocation across all 6 cases (both present → rc=0; each one missing; both missing → **both** errors reported, not just the first). Parity test tightened: it pinned `compgen -G`, which kept passing on a file whose only `compgen` was in a COMMENT — it now asserts the step exists, can emit `::error::`, and prints its verdict. ⭐ **RESOLUTION (2026-08-13) — `~/.bash_logout` was overwriting the exit status.** [bash(1) INVOCATION](https://www.gnu.org/software/bash/manual/html_node/Bash-Startup-Files.html) (checked locally in `/usr/share/man/man1/bash.1.gz:347`): *"When an interactive login shell exits, **or a non-interactive login shell executes the `exit` builtin command**, bash reads and executes commands from the file `~/.bash_logout`."* `bash -el {0}` is a non-interactive **login** shell, and that step was **the only step in the action that called `exit`** — so it was the only one that sourced `~/.bash_logout`, whose last command's status then replaced its own. On an Ubuntu runner that file is the `/etc/skel` default ending in `[ -x /usr/bin/clear_console ] && /usr/bin/clear_console -q`; `SHLVL` is 1 (bash is spawned by a non-shell parent, so bash sets it itself) and there is no terminal, so it exits 1 → **the step exits 1 no matter what the script decided.** **Probe (Debian/WSL, bash 5.2.37, `SHLVL` unset in the parent, skel `.bash_logout` in `HOME`):** `bash -el` + `exit 0` → **1** (the CI failure, reproduced); `bash -el` with no `exit` builtin → 0 (why every other `-el` step passed); `bash --noprofile --norc -eo pipefail` + `exit 0` → 0 (the fix); `bash -el` + `exit 1` → 1 (real failures still fail); `bash -el` + `exit 0` with **no** `.bash_logout` → 0 (**why the earlier local repro "passed" — its HOME had no such file; the two deciding variables, `~/.bash_logout` and `SHLVL`, are both OUTSIDE the script, so re-reading the script was never going to find it**). So the earlier variable-elimination fix was **correct but for the wrong reason** — it works because it drops the *login shell*, not because of `compgen`. **THE RULE: a login-shell step must never call `exit`** — let the script fall off its end, or push the deciding logic into a called script (`bash foo.sh` is a child, non-login shell — which is why `verify-payload.sh` was never affected), or drop `-l`. **New `tests/test_workflow_shell_hygiene.py`** enforces it across both workflows and both composite actions, honouring job-level `defaults.run.shell` inheritance; **negative control**: the detector run against the pre-fix `action.yml` at `7d95a40` flags exactly that one step, and HEAD is clean. Loud ⛔ comments added at the `defaults.run.shell: bash -el {0}` sites in `ci.yml` + `release.yml` and in both actions. **Re-verified end-to-end:** both assertion scripts extracted from the YAML and run in WSL under GitHub's exact `shell: bash`, in a HOME carrying the skel `.bash_logout` (proving it inert) — 6/6 cases (both present → 0; either missing → 1; nothing built → 1). ❓ **Not yet confirmed on a runner** (this branch is unpushed) — and the runner's `/home/runner/.bash_logout` contents are inferred from `/etc/skel`, not read; the fix does not depend on which command in it fails | live-test-pending | **P1** | agent | `Reference/packaging.md` § *The PR gate runs the RELEASE build*; `tests/test_workflow_shell_hygiene.py` |
| CS-163 | **Merging `main` → `release/0.1` created NO release — the workflow was never QUEUED (not a failed run). Diagnosed 2026-08-13.** `release.yml` fires on `push` to `release/*` **filtered to `paths: ['src/__version__.py']`**. Per [GitHub's workflow-syntax docs](https://docs.github.com/en/actions/reference/workflows-and-actions/workflow-syntax#onpushpull_requestpull_request_targetpathspaths-ignore), a push **to an existing branch** evaluates that filter against a **two-dot diff of the push's before/after SHAs** — so the question is not "did any pushed commit touch the file" but "do the two tips DIFFER in it". **`src/__version__.py` has not changed since `e86022e` (v0.1.0-alpha.1):** blob `bf3b7577` is byte-identical on `e86022e`, `a9ede1c` (release/0.1), `6724238` (main) and `a491854`. The merge changed **32 files, none of them that one** → no match → no run, no red X, no feedback anywhere. **Positive control:** the merge that DID fire (PR #2, `7353006`→`a9ede1c`) has `src/__version__.py` in its two-dot diff. **So this is working as designed** — CONTRIBUTING § *Triggering a release* says exactly this ("non-version commits on a release branch do not trigger a release") — but the failure mode is SILENT: `ci.yml`'s `version-check` validates that `__version__`'s `X.Y` MATCHES the target release branch, never that it was BUMPED, so the PR goes green and merges with nothing downstream. **To release:** bump `__version__` (e.g. `0.1.0-alpha.2`) and merge that into `release/0.1`; or `workflow_dispatch` the Release workflow **with `release/0.1` selected** (the `version` job forces `should_release=true` on dispatch precisely so a broken release can be re-run, and dispatch runs the workflow file **as it exists on the selected ref**). ⚠️ Re-releasing `v0.1.0-alpha.1` would put a tag minted for `e86022e`'s content on a tree that has since gained the `.deb` fixes, the full-release PR gate, Simplify Bodies and `CS-161` — prefer a new prerelease number. ❓ **Unverified from here:** the remote tag list, and whether a `v0.1.0-alpha.1` release exists — `git fetch` fails in the agent shell with `Bad owner or permissions on C:\Users\mmccormick/.ssh/config` (an ssh-config ACL issue in MY shell only; the user's own terminal authenticates fine), so the `origin/*` refs cited above are as of the last successful fetch. ⭐ **FIXED THE SAME DAY, user-chosen (both halves):** (1) **the `paths` filter is GONE** — `release.yml` now fires on EVERY push to `release/*`. Safe because the dedupe already existed one layer down: the `version` job clears `should_release` when the tag is already on the remote, and both build jobs are `if:`-gated on it, so a push that cannot release costs one ~15 s job and leaves a `::notice::` instead of silence. ⚠️ Flip side: a push to a release branch whose version is NOT yet tagged **will publish, whatever files it touched** — tag state decides, not paths. (2) **`ci.yml`'s `version-check` now refuses a PR into `release/X.Y` whose version is already tagged** (`git ls-remote` + `::error::`), so the no-op merge is caught BEFORE it lands. ⚠️ That makes **syncing a release branch with `main` without releasing no longer a single merge** — bump the version or land the sync elsewhere; the accepted trade. (3) **Hardening:** all four remote lookups switched from `git ls-remote … | grep -q` to assign-then-test — a piped lookup reads a NETWORK FAILURE as "tag absent" (and under `pipefail`, `grep -q`'s early exit can SIGPIPE the producer and fail a successful pipeline); with the assignment `set -e` aborts on a failed query, the safe direction. Not an observed failure — same family as `CS-159`. **New `tests/test_release_trigger.py`** pins the no-`paths` trigger, the `workflow_dispatch` escape hatch, the `should_release` gate on both build jobs, the PR-time guard, and the no-`grep -q` rule; **mutation-tested 6/6** (re-add `paths` · delete the tag lookup · ungate `build-linux` · drop the PR guard · reintroduce `ls-remote | grep -q` as code · remove `workflow_dispatch`) with a green baseline. Its `grep -q` detector scans `run:` scripts with comment lines stripped — the first version flagged the comments WARNING about the anti-pattern, the same way the parity test once matched a `compgen` that survived only in a comment. Full tier: **77 passed, 1 deselected**. Docs updated: CONTRIBUTING § *Triggering a release* (trigger prose + mermaid + the new PR check), `Reference/packaging.md` (trigger row + a new section). ❓ **Runner-unverified** — nothing here has run on GitHub yet | done | P2 | agent | `.github/workflows/release.yml` trigger block; `ci.yml` `version-check`; `tests/test_release_trigger.py`; CONTRIBUTING § *Triggering a release* |
...
-19. **Component Editor: operation-sequence model + lineage tree (9b/9e) — code
   ...
   - **Merge-ALL-into-one now Applies (FIXED; headless-verified, GL live-test
     PENDING):** merging every body in the Component Editor into one left the
     window's Apply button DISABLED — `_update_apply_enabled` gated on
     `len(self._final_lids) >= 2`, and `SplitRecipe.is_effective()` gated on
     `len(final_ids()) >= 2 or any delete op`. So a 1-final-body recipe was
     rejected at the GUI *and* would have dropped on `dump_split_map` (edit
     vanishes + reverts on reload). The count was never the right test: **merge-all
     fuses N solids into one body** (a real structural edit — the "collapse this
     assembly into a single solid" workflow), same as delete-to-one and a transform
     of a lone body. `is_effective()` is now **`len(final_ids()) >= 2 or
     bool(ops)`** — the delete special-case is subsumed; only a genuine no-op (one
     body, zero ops) is rejected. `_update_apply_enabled` + `_on_apply` now call
     that SAME predicate (one source of truth with the bake's
     `resolve_split_targets`, whose error text changed from "yields fewer than 2
     bodies" to "is a no-op"). Verified headlessly: merge-all recipe is effective,
     round-trips through `dump_split_map`/`load_split_map`, and `run_body_ops` on a
     3-box compound returns one final body (`o0:0`) containing 3 solids. Bake side
     needed NO change — `run_body_ops`, `run_body_ops_mesh`, `apply_splits`
     (leaf in-place conversion AND assembly `_replace_assembly_children`) and
     `split_stub_assembly` are all count-generic; delete-to-one already proved
     1-final-body bakes. Side effect: a **transform-only recipe on a single-solid
     leaf** is now applicable too (it was silently un-Appliable before).
