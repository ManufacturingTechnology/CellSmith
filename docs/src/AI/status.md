...
>
> **PARTIAL UPDATE (2026-08-04):** a `build-linux` run HAS now happened. It got all the way to the payload verifier, which passed **every** check (Qt platform plugin, 150 OCCT libs, OCCT resources, 112 pxr files, 382 VTK libs, notices + LICENSE + OFL, `--version`, all 12 workers) **except the readline assertion — 3 hits → `CS-157`**. So the freeze itself, the Linux system-library list, and the worker/hiddenimports derivation are now runner-confirmed; the `pythonpath` fix and the eight GPL rows in `test-linux` are still unread.
